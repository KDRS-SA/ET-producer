...
from tkinter import Tk, messagebox, filedialog, Label, Button, Entry, ttk, StringVar, simpledialog, Menu
...
def configure_sip_premis(premis_path: str, id: str, tarfolder: str):
...
def configure_sip_mets(mets_path: str, id: str, tarfolder: str, creation_date: str, premis_path: str):
...
def validation_func(val: str, rem: str) -> bool:
    if rem == "1":
...
def combo_helper(element: dict, list: list):
    element['values'] = [i for i in list if element.get().lower() in i.lower()]

def set_username():
    global USERNAME
    val = simpledialog.askstring(title=' ', prompt='Set your username:', initialvalue=USERNAME)
    if val:
        USERNAME = val
        tmp_menu.entryconfigure(1, label=f'Username: {USERNAME}')
...
#Creates all GUI widgets and adds input widgets to a list
...
tmp_menu = Menu(window)
tmp_menu.add_command(label=f'Username: {USERNAME}', command=set_username)
window.config(background="teal", menu=tmp_menu)
